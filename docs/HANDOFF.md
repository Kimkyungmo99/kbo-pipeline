...
9. **투수·타자 단위 분석 마트 완료** — `agg_pitcher_season` 1,304행, `agg_batter_season` 1,427행. 양쪽 투구 합계 483,164구·타석 종료 합계 124,334건 일치. 타석 결과는 종료 투구만 집계해 중복 방지. dbt build 63개 항목, pytest 38개 통과.
10. **선수 이름·팀 이력 완료** — relay raw의 선수 레코드·교체 정보와 실제 투구 상태를 결합해 날짜별 51,984개 관측을 생성하고 R2 `bronze_players/`에 저장. `dim_players` 809명 이름 전원 연결, `dim_player_team_seasons` 1,532행 생성, 같은 시즌 복수 팀 16명 식별. 실제 투구 옵션만 소속 근거로 사용해 이닝 전환의 stale 상태 오귀속을 차단. dbt build 82개 항목, pytest 39개 통과.
11. **자동화 off-day 결함 수정** — 완료 경기가 없는 날 bronze 폴더가 없어 완전성 검사가 실패하던 문제 수정. 실패 run 35675215892의 이슈 #1에 원인·해결을 기록하고, 같은 날짜 재실행 35797137102 성공 후 종료.
12. 그 다음: 선수 이름을 투수·타자 집계 마트에 직접 노출하고 팀 단위 분석 마트 설계. 새 시즌마다 `SEASON_OPENING` 등록.
13. 문서 후속: ADR 002 개정 2(시즌 간 규정 변화가 검증 빈도에 나타난 사례), ADR 003에 exhibition·SEASON_OPENING 보강, 면접 문답 Q9(시즌별 변화율 조기 경보)·Q10(규모가 커져야 드러나는 운영 결함 — 1,009번째 파일).
