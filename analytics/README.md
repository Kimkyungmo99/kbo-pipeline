...
- `stg_player_observations`: relay raw에서 복원한 날짜별 선수 이름·팀·투타 역할 관측. 날짜별 이름 누락은 다른 경기의 동일 선수 ID로 보완한다.
...
- `dim_player_team_seasons`: 선수의 시즌별 소속 팀과 최초·최종 관측일. 시즌 중 이적은 팀별 행으로 유지한다.
