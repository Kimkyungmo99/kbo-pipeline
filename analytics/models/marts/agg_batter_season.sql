with pitch_metrics as (
    select
        ...
        avg(velocity) filter (where velocity > 0) as avg_velocity_seen,
    ...
    group by season, game_type, batter_id
),
plate_appearance_metrics as (
    select
        ...
        count(*) filter (where plate_appearance_result = 'single') as single_count,
        count(*) filter (where plate_appearance_result = 'double') as double_count,
        count(*) filter (where plate_appearance_result = 'triple') as triple_count,
...
select
    ...
    plate_appearance_metrics.single_count
        + plate_appearance_metrics.double_count
        + plate_appearance_metrics.triple_count
        + plate_appearance_metrics.home_run_count as hit_count,
